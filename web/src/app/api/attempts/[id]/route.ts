...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // R53/R55/R56: ONE predicate, built once here, decides BOTH which runs reach
  // the ranking and which runs the response marks as not counted. They are the
  // same expression, so they cannot drift apart -- and it lives outside the
  // completion block because the dashboard needs the flag on every poll,
  // including polls of an attempt that completed on an earlier one.
  //
  // Two independent reasons a run does not count, and both belong here (R56):
  // a platform fault (spec L126 -- infra luck is excluded so it can never cap
  // a score) and having no round-1 run at all (R10 -- deactivated
  // mid-attempt, or platform-errored in round 0 and skipped at round-1
  // fan-out). A field named excludedFromRanking that reads false for a run
  // that IS excluded would be worse than no field: Task 19 would get the math
  // strip wrong precisely by reading it correctly.
  //
  ...
  if (attempt.status === "active" && allTerminal(r0) && allTerminal(r1)) {
    ...
    // R56: BOTH rounds are filtered by the same predicate the flag uses, so
    // the array handed to scoreAttempt and the serialized flag cannot
    // disagree. scoreAttempt still drops platform runs itself -- that filter
    // is now a redundant backstop rather than the only thing making the flag
    // true, which is the point: the flag no longer depends on a rule owned by
    // another module. Numbers are unchanged either way (verified).
    const out = scoreAttempt(
      toScored(r0!.runs.filter(counted)), toScored(r1!.runs.filter(counted)),
      attempt.challenge.parTokens);
  ...
  // R55/R56: a computed field, added after the query, never part of the
  // select -- it says whether the run COUNTS toward its round's ranking, not
  // what it scored. An excluded run keeps its real runScore in the payload: spec L20
